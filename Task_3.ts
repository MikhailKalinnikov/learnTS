...
const normalizeData = (posts: IPost[]) => {
  const byIdArr: string[] = posts.map((el: IPost) => el.id);
  const allId: {} = { allIds: byIdArr };

  const preId = posts.map((el: IPost) => el);
  let preIdTitle = [];
  for (let i = 0; i < preId.length; i++) {
    preIdTitle.push({ [preId[i].id]: preId[i] });
  }
  const resultById: { byId: {} } = { byId: Object.assign({}, ...preIdTitle) };
  const totalResult = Object.assign({}, resultById, allId);
  return totalResult;
};
